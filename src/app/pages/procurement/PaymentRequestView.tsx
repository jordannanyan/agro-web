...
import { ArrowLeft, CreditCard, Banknote, Copy, Check, Landmark, FileSpreadsheet } from "lucide-react";
...
export default function PaymentRequestView() {
  ...
  // There is one way to settle a request and this page is not it: finance transfers
  // quoting the code below, then uploads the statement, and the matching line marks
  // it Paid. The manual override that used to sit at the bottom of this page is gone
  // -- POST /payment-requests/:id/pay survives as a Super-Admin-only break-glass that
  // no screen offers, so nobody is invited to settle a payment by asserting one.
        ...
        {data && (
          <>
            ...
            <Card className="p-6">
              ...
              {/* Where the money actually goes. This used to be one grey line of 12px
                  text under the amount, which is the wrong size for the one field on
                  the page that is copied into a bank: an account number misread by a
                  digit pays a stranger, and there is no undo. So it gets its own
                  block, the number gets a monospace face and a copy button, and an
                  empty one says so instead of quietly rendering nothing. */}
              <div className="mt-4 border border-slate-200 rounded-xl overflow-hidden">
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 border-b border-slate-200">
                  <Landmark className="w-3.5 h-3.5 text-slate-400" />
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Rekening Tujuan</p>
                </div>
                {data.bank_account || data.bank_name ? (
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-slate-100">
                    <div className="bg-white px-4 py-3">
                      ...
                      {/* The name the document was approved with, not the master's
                          current one — a bank renamed later must not silently rewrite
                          a transfer somebody already signed. */}
                      <p className="text-sm font-medium text-slate-800">{data.bank_name || data.bank_list_name || "—"}</p>
                      {data.bank_code && <p className="text-[11px] font-mono text-slate-400 mt-0.5">{data.bank_code}</p>}
                    </div>
                    <div className="bg-white px-4 py-3">
                      <p className="text-xs text-slate-400 mb-1">Nomor Rekening</p>
                      <div className="flex items-center gap-2">
                        <p className="text-base font-mono font-semibold tracking-wide text-slate-900 break-all">{data.bank_account || "—"}</p>
                        {data.bank_account && (
                          <button
                    ...
                    <div className="bg-white px-4 py-3">
                      <p className="text-xs text-slate-400 mb-1">Atas Nama</p>
                      <p className="text-sm font-medium text-slate-800">{data.beneficiary_name || "—"}</p>
                    </div>
                  </div>
                ) : (
                  <p className="px-4 py-3 text-sm text-amber-800 bg-amber-50">
                    Rekening tujuan belum diisi — permintaan ini belum bisa ditransfer maupun masuk file export Kopra.
                  </p>
                )}
              </div>
            </Card>
